...
namespace realm
{
	namespace io
	{
			...
			const std::string& opk_filepath)
		{
			...
			double omega, phi, kappa = 0;
			RotationMatrix_To_EulerAngleOPK(R, omega, phi, kappa);
			...
			if (opk_filepath.length() > 0)
			{
				std::ofstream opk_file;
				opk_file.open(opk_filepath.c_str(), std::ios::app);
				saveTrajectoryOPK(&opk_file, timestamp, t.at<double>(0), t.at<double>(1), t.at<double>(2), omega, phi, kappa, R);
				opk_file.close();
			}
		}
		...
		void saveTrajectoryOPK(std::ofstream* file, uint64_t timestamp, double x, double y, double z, double omega, double phi, double kappa, const cv::Mat& R)
		{
			// TUM file format
			(*file).precision(10);
			(*file) << timestamp << " " << x << " " << y << " " << z << " " << omega << " " << phi << " " << kappa <<
				" " << R.at<double>(0, 0) << 
				" " << R.at<double>(0, 1) << 
				" " << R.at<double>(0, 2) << 
				" " << R.at<double>(1, 0) << 
