...
namespace MyREALM
{
	...
	void SparseReceiverThread::setVertics(const osg::Vec3Array& arr)
	{
	...
	bool SparseReceiverThread::getVertics(osg::Vec3Array& arr)
	{
		OpenThreads::ScopedLock<OpenThreads::Mutex> lock(_mutex);
		if (_dirty)
		{
	...
	void SparseDrawCallback::update(osg::NodeVisitor* nv, osg::Drawable* drawable)
	{
		...
		*/
		...
		if (vertices && m_thread && m_thread->getVertics(*vertices_add.get()))
		{
			if (vertices_add->size() > 0)
			{
				int insert_pos = vertices->size();
				vertices->insert(vertices->end(), vertices_add->begin(), vertices_add->end());
				osg::ref_ptr<osg::DrawArrays> drawPts = new osg::DrawArrays(
					osg::PrimitiveSet::POINTS, insert_pos, vertices_add->size());
				geom->addPrimitiveSet(drawPts);
