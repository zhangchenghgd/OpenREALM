...
namespace MyREALM
{
	osg::ref_ptr<osg::Geode> createTrajGeode(const osg::Vec4& color,
		float line_width, TrajDrawCallback* clb)
	{
	...
	TrajReceiverThread::TrajReceiverThread() : OpenThreads::Thread()
	{
		_lineVertics = new osg::Vec3Array;
	}

	int TrajReceiverThread::cancel()
	{
		_done = true;
	...
	void TrajReceiverThread::setVertics(const osg::Vec3Array& arr)
	{
		OpenThreads::ScopedLock<OpenThreads::Mutex> lock(_mutex);
		_lineVertics->clear();
		_lineVertics->insert(_lineVertics->begin(), arr.begin(), arr.end());
		_dirty = true;
	}

	bool TrajReceiverThread::getVertics(osg::Vec3Array& arr)
	{
		OpenThreads::ScopedLock<OpenThreads::Mutex> lock(_mutex);
		if (_dirty)
		{
			arr = *_lineVertics.get();
			//arr.insert(arr.end(), _lastLineVertics->begin(), _lastLineVertics->end());
			_dirty = false;
			return true;
		}
		return false;
	...
	TrajDrawCallback::TrajDrawCallback(TrajReceiverThread* p_thread) :
		osg::DrawableUpdateCallback(), m_thread(p_thread)
	{
	}

	TrajDrawCallback::~TrajDrawCallback()
	{
	}

	//void update(NodeVisitor *, Drawable *)虚函数，重构后可以实时获取和操作几何体
	//参数：系统内置的访问器对象，用于遍历节点树并找到回调对象的位置；回调所在几何体的对象指针，以便对其进行操作
	void TrajDrawCallback::update(osg::NodeVisitor* nv, osg::Drawable* drawable)
	{
		...
		*/
		...
		if (vertices && m_thread->getVertics(*vertices))
		{
			osg::ref_ptr<osg::DrawArrays> drawPts = new osg::DrawArrays(
				osg::PrimitiveSet::LINE_STRIP, 0, vertices->size());
